...
public class DULong : DType<ulong>
{
    public static DULong Default { get; } = new(0);
    private const int POOL_SIZE = 128;
    private static readonly DULong[] s_pool = new DULong[POOL_SIZE];

    static DULong()
    {
        for (ulong i = 0; i < POOL_SIZE; i++)
        {
            s_pool[i] = new DULong(i);
        }
    }

    public static DULong ValueOf(ulong x)
    {
        if (x >= 0 && x < POOL_SIZE)
        {
            return s_pool[x];
    ...
    {
        return obj is DULong o && o.Value == this.Value;
    }
